const puppeteer = require('puppeteer');
const DeathByCaptcha = require("deathbycaptcha");
// const ProxyFinder = require('public-proxy-finder');
const self = this;
var browser;

var pageInstances = [];
exports.pageInstances = pageInstances;

function readCaptcha(buf){
    return new Promise((resolve,reject) => {
        try{
            // console.log("Reading Captcha...");
            const dbc = new DeathByCaptcha("sascha1337", "ASDASDASD123");
            dbc.solve(buf, function(err, id, solution) {
                if(err) return console.error(err); // onoes!
                // console.log(err, id, solution);
                if(solution){
                    captcha = solution;
                    resolve( solution );
                }else{
                    console.log("NOT FOUND");
                    reject();
                }
            });
        }catch(ex){
            console.log("xxxx+++++++", ex);
            reject();
        }
    })
}

function readAntiCaptcha(buf){
    // http module should be installed:
    // npm i http

    // Params:
    // your anti-captcha.com account key

    return new Promise((resolve,reject) => {
            
        var anticaptcha = require('./anticaptcha')('02bd85c073bdad4c621930fa8904ecac');

        // check balance first
        anticaptcha.getBalance(function (err, balance) {
            if (err) {
                console.error(err);
                return;
            }
            
            // captcha params can be set here
            anticaptcha.setMinLength(5);
            
            if (balance > 0) {
                anticaptcha.createImageToTextTask({
                        case: false, // or params can be set for every captcha specially
                        body: buf.toString('base64') //''
                    },
                    function (err, taskId) {
                        
                        if (err) {
                            console.error(err);
                            reject(err);
                        }

                        // console.log(taskId);
                        
                        anticaptcha.getTaskSolution(taskId, function (err, taskSolution) {
                            if (err) {
                                console.error(err);
                                reject(err);
                            }

                            resolve(taskSolution);
                            console.log(taskSolution);
                        });
                    }
                );
            }else{
                console.log("**** BALANCE FAIL !!!! ****");
            }
        });
    });

}

// readAntiCaptcha(null);

async function crackCaptcha(page, res) {

    await page.waitFor("#c_verifyimei_botdetectcaptcha_samplecaptcha_CaptchaImage");
    const captchaElement = await page.$('#c_verifyimei_botdetectcaptcha_samplecaptcha_CaptchaImage');
    const buf = await captchaElement.screenshot();
    const cap = await readAntiCaptcha(buf);
    await page.type("#BotDetectCaptcha_CaptchaCodeTextBox", cap);
    await page.click("#BotDetectCaptcha_ValidateCaptchaButton");
    await page.waitFor(4000);
    // await page.waitFor(3000);
    // await page.screenshot({ path: 'example.png', fullPage: true });
    const captcha_err = await page.evaluate(() => document.querySelector('#BotDetectCaptcha_CaptchaIncorrectLabel').style.display);
    if (captcha_err === "inline-block") {
        res = false;
    }
    else {
        res = true;
    }
    return res;
}

exports.checkIMEI = async function(imei, proxy) {
    const page = await browser.newPage();
    
    try {
        
        pageInstances.push({imei, page});

        page.on('error', err=> {
            console.log('error happen at the page: ', err);
        });
        
        page.on('pageerror', pageerr=> {
            console.log('pageerror occurred: ', pageerr);
        })
    
        await page.goto('https://www.t-mobile.com/verifyIMEI.aspx');
        // console.log("navigating");
        await page.type("#imei", imei.toString() );
        await page.click("#btnSubmit")
        await page.waitFor("#c_verifyimei_botdetectcaptcha_samplecaptcha_CaptchaImage")
        // await page.screenshot({path: 'captcha' + imei + '.png', fullPage: false});

        var res = false;
    
        while(res !== true){ 
            // console.log("captcha run");
            res = await crackCaptcha(page, res);
        }
    
        // console.log("+1 captcha success");
    
        const captcha_err = await page.evaluate(() => document.querySelector('#BotDetectCaptcha_CaptchaIncorrectLabel').style.display);
        const valid = await page.evaluate(()=> document.querySelector('#pnlActive'));
    
        // console.log(valid);
        var response = null;
        
        if(valid){
            // console.log( "ready" );
            response = "ready";
            
        }else{
    
            var alert = await page.evaluate(()=> document.querySelector('.alert').textContent);
            // console.log("IMEI::" + imei + "::" + alert);
            
            if(alert.includes("outstanding balance that")){
                // console.log("outstanding_balance")
                response = "outstanding_balance";
            }else if(alert.includes("has been reported stolen")){
                // console.log("stolen")
                response = "stolen";
            }else if(alert.includes("device is blocked")){
                // console.log("device is blocked")
                response = "device is blocked";
            }else if(alert.includes("do not recognize the IMEI")){
                response = "unrecognized";
            }else if(alert.includes("been reported lost")){
                response = "lost";
            }else if(alert.includes("without dashes or spaces")){
                response = false;
            }else{
                // console.log("::unknown::", alert);
                response = "unknown::" + alert;
            }
            
            // return returnCode;
        }
    
        // await browser.close();
        page.close();
        return response;
    }catch(ex) {
        console.log("##ääääxxxxx::::", ex);
        page.close();
        return false;
    }
    // }catch(ex){
    //     console.log("x0x0x0x00x0x0x00x", ex);
    //     return false;
    // }

}

exports.init = async function(){
    browser = await puppeteer.launch();
    return true;
}